...
public class EnemyRunState : EnemyBaseState
{
    ...
    private bool scanning, startKick;
    ...
    private void ScanArea()
    {
        ...
        for (int i = 0; i < hits; i++)
        {
            ...
            {
                if((GameManager.instance.XROrigin.transform.position - colliders[i].transform.position).magnitude < (agent.transform.position - colliders[i].transform.position).magnitude)
                {
                    Debug.Log(colliders[i] + "Closer than the player");
